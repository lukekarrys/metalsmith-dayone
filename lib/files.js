    ...
    if (isMatch) {
      ...
      entries.forEach((entry) => {
        ...
        .replace(rPhoto, (__, id) => {
          ...
          // Add the path to the photo within the entry
          photoInfo.path = photoPath

          // Add the photo back to the output since its referenced
          createFile(photoPath, photos[photoPath])

          const dimensions = parseMarkdown && photoInfo.width && photoInfo.height ? ` =${photoInfo.width}x${photoInfo.height}` : ''

          // And finally rewrite the src
          return `/${photoPath}${dimensions}`
        })
        ...
        const data = Object.assign({}, entry, {
          contents: Buffer.from(parseMarkdown ? html : md),
          layout: entryLayout,
