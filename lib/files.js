    ...
    if (isMatch) {
      ...
      entries.forEach((entry) => {
        ...
        .replace(rPhoto, (__, id) => {
          ...
          const { md5, width, height } = _.find(entry.photos, { identifier: id }) || {}
          // Double check to make sure that the photo exists
          const photoPath = _.findKey(photos, (photo, id) => id.startsWith(photosPath + md5))
          ...
          const dimensions = parseMarkdown && width && height ? ` =${width}x${height}` : ''
